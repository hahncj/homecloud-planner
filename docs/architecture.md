...
- `auth` — the single local admin account (`AdminUser`, BCrypt via `PasswordEncoder`), session-cookie login/logout/session endpoints, Spring Security configuration (CSRF, CORS wiring, RFC 9457 401/403 responses). See [ADR-0008](decisions/ADR-0008-authentication-approach.md).
...
Authentication follows the same mirrored-module pattern: `auth/AuthContext.tsx` wraps the session-check, login, and logout TanStack Query calls (`api/auth.ts`) behind a `useAuth()` hook; `auth/ProtectedRoute.tsx` gates every authenticated route (nested under it in `App.tsx`) and redirects to `pages/auth/LoginPage.tsx` when there's no session. `api/client.ts` attaches `credentials: 'include'` and the `X-XSRF-TOKEN` CSRF header to every request and exposes a module-level unauthorized-response hook so a 401 from *any* query (not just the session check) clears cached auth state — see [ADR-0008](decisions/ADR-0008-authentication-approach.md).
